    ...
    return (
        <div className="h-full rounded-md ring-1 ring-slate-200/40 bg-white p-4 grow">
            <div className="mb-4 flex gap-3">
                <Tabs $active={!tabs} onClick={() => setTabs(false)} > Upcoming </Tabs>
                <Tabs $active={tabs} onClick={() => setTabs(true)} > Active </Tabs>
            </div>
            {
                !tabs ? 
                    <>
...
const Tabs = styled.button.attrs(({$active}) => ({
    type: "button",
    className: `px-3 h-8 rounded-md ${$active?'bg-blue-500/20 text-blue-600':'hover:bg-blue-600/5'}`
}))``