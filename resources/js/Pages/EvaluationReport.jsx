...
import TrainingSummaryDataDetails from "@/Components/Reports/chart_details/TrainingSummaryData";
import ResourcePersonDataDetails from "@/Components/Reports/chart_details/ResourcePersonData";
    ...
    return (
        <Reports auth={auth} report={report}>
            ...
            <div className="container mt-3 sm:p-4 py-4">
                ...
                <div className="sm:max-h-fit max-h-[60vh] max-xs:overflow-y-auto sm:mt-0 mt-3 sm:p-0 px-4 max-xs:pb-1">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-3 max-xs:mt-1">
                        <div className="container !ring-slate-200/80 p-3 flex flex-col relative">
                            {evaluationData && (
                                <DetailsButton
                                    onClick={() => setData("consolidate", true)}
                                />
                            )}
                            ...
                            <div className="h-[18rem] my-auto">
                                ...
                                >
                                    <ConsolidatedDataChart
                                        consolidated={
                                            evaluationData?.consolidatedChart
                                        }
                                    />
                                </LoadingContent>
                            </div>
                            {evaluationData && (
                                <div className="text-center text-xs">
                                    ...
                                    participants gave the training activity a
                                    rating of 4 or 5, with over{" "}
                                    {consolidatedPercent}% of participants
                                    giving a rating of 4 or higher. <br />
                                    This suggests that the activity was{" "}
                                    {consolidatedPercent <= 40
                                        ? "less "
                                        : consolidatedPercent <= 41 &&
                                          consolidatedPercent >= 60
                                        ? "moderately "
                                        : "well-"}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="container !ring-slate-200/80 p-3 flex flex-col relative">
                            {evaluationData && !loadingRp && (
                                <DetailsButton
                                    onClick={() => setData("rp", true)}
                                />
                            )}
                            ...
                            <div className="h-[18rem] my-auto">
                                ...
                                >
                                    <ResourcePersonResultChart
                                        evaluation_ratings={
                                            evaluationData?.rpRatingSummary
                                        }
                                    />
                                </LoadingContent>
                            </div>
                ...
                <PrintEvaluations
                    src={route("print.charts", [selectedEvent ?? "null"])}
                    show={isPrintChart}
                    onCancel={() => setIsPrintChart(false)}
                />
            </div>
            ...
            <Consolidated
                ...
                initialData={evaluationData?.consolidated}
            />
            <QualitativeAssessment
                isPrintable={selectedEvent ?? false}
                eventId={selectedEvent}
                initialData={evaluationData?.eventAssessment}
            />
            ...
            <ConsolidatedDataDetails
                data={evaluationData?.consolidatedChart}
                show={data.consolidate}
                onClose={() => setData("consolidate", false)}
            />
            <TrainingSummaryDataDetails
                data={evaluationData?.trainingActivityRatingSummary}
                show={data.training}
                onClose={() => setData("training", false)}
            />
            <ResourcePersonDataDetails
                data={evaluationData?.rpRatingSummary}
                show={data.rp}
                onClose={() => setData("rp", false)}
            />
        </Reports>
    );
}
...
const DetailsButton = ({ ...props }) => (
    <button
        {...props}
        className="absolute top-1 right-1 w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center"
    >
